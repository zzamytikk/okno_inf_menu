var Oinf = {
  ...
  x: 30,//сек до автозакрытие окна (При 30 секундном бездействии окна) Новое смс обновит таймер!
  /*y: 40,      //px {number}
    y: '2.5rem',//{string} */
    ...
    $: function(q,X) {//Добовляем сообщение?. || Заменим на новое
      ...
      $(O.id).append(//Добавляем сообщение
        $('<div>').attr({
          ...
          .html('<div'+(X?' style="display:block"':'')+'>'+q.t+'</div>'+(q.a>=0?'<span></span>':''))
